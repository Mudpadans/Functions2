...
function contains(array, names, callback) {
  if (array.find(element => element === names)) {
    callback(true);
  } else {
    callback(false);
  }
...
// CODE HERE
/*each(names, indexStatement = (item, index) => {
...
function addingFactory(num1) {
  return function addingMachine1(num2) {
    let sum = num1 + num2;
    console.log(sum);
  }
}
...
function getUserById(arr, id, callback) {
  let targetObj = arr.find(obj => obj.id === id)
  callback(targetObj);
}
